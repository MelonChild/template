                                ...
                                <?php if(!empty($list)): if(is_array($list)): foreach($list as $key=>$user): ?><tr>
                                        ...
                                        <td>
                                         <?php if($user['username'] != C('RBAC_SUPERADMIN')): ?><div class="btn-group"> 
                                                <?php if($user["is_active"] == 1): ?><a class="btn btn-small" rel="tooltip" data-placement="left" data-original-title=" 锁定 "  href="<?php echo U('Admin/Rbac/setActive',array('id'=>$user['id'],'value'=>0));?>">锁定</a>
                                                    <?php else: ?> 
                                                    <a class="btn btn-small" rel="tooltip" data-placement="left" data-original-title=" 启用 "  href="<?php echo U('Admin/Rbac/setActive',array('id'=>$user['id'],'value'=>1));?>">启用</a><?php endif; ?>
                                                <a class="btn btn-small" rel="tooltip" data-placement="left" data-original-title=" 编辑 " href="<?php echo U('Admin/Rbac/editUser',array('id'=>$user['id']));?>">编辑</a> 
                                                <a class="btn  btn-small" rel="tooltip" data-placement="bottom" data-original-title="删除"  onclick="return confirm('是否确定要删除?');" href="<?php echo U('Admin/Rbac/deleteUser',array('id'=>$user['id']));?>">删除</a> 
                                            </div><?php endif; ?>
                                        </td>
